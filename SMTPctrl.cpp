...
static char					*DatabaseConnection = "SpamHater Database",*BlankTagLine = "-";
...
void TranslateEncodedChars(CLIENT_CONTEXT *lpClientContext,char *Source)
{
	...
	if (strlen(Source) > 0)
	{
		...
		SearchAndReplace(lpClientContext->MessageBody,"æ","ae",NULL);
		SearchAndReplace(lpClientContext->MessageBody,"Æ","ae",NULL);
		SearchAndReplace(lpClientContext->MessageBody,"\t"," ",NULL);
...
void TranslateAnnotatedChars(char *Source)
{
	...
	strcpy_s(AnnotatedChars,sizeof(AnnotatedChars),"`´\t\r\nåãâáàäÅÄÃÂÁÀêéèëËÉÈÊîíìï¡ÏÎÌÍôóòöõøÖÒÓÔÕØð¤ûúùüÜÙÚÛµñÑýÿÝ¥çÇ¢ßÐ¿þÞ£§¶©®¨±×÷«»°º¹²³·ª¬­¯¸¼½¾¦|");
